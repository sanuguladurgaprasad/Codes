...
struct btree
{
  int *key;
  btree **child;     
};int p=0,a1;btree *b1=NULL,*b2=NULL,*x,*z;
...
btree* split(btree *b,int k,int d,int f)
{int u=0,v=0;cout<<"s";
     ...
     if(k==e->key[i]){e->child[i]=b1;e->child[i+1]=b2;}
             ...
             else{e->child[i+1]=b->child[i];}}//if(e->child[i]!=NULL){cout<<"W";print(e->child[i],d);cout<<"Q";}
     }*/
...
{int count=0,kount=0,s=0;
     if(b!=NULL)
     {
         ...
         {cout<<"p";
             ...
             {cout<<"a";if(b==h1){b=split(b,k,d,1);break;}
               else{p=1;cout<<"z";split(b,k,d,0);}
             }cout<<p<<"P";
             /*if(p==2)
             {cout<<"R";
                  if((count==2*d)&&(kount!=0))
                  {cout<<"c";
                        if(b==h1){b=split(b,a1,d,1);}
                        else{p=1;cout<<"z";split(b,a1,d,0);}
                  }
                  else if((count!=2*d)&&(kount!=0))
                  {s=0;cout<<"W";
             ...
             {cout<<"R";
                  if((count==2*d)&&(kount!=0))
                  {cout<<"c";
                        if(b==h1){cout<<"G";b=split(b,a1,d,1);}
                        else{p=1;cout<<"z";split(b,a1,d,0);}
                  }
                  else if((count!=2*d)&&(kount!=0))
                  {s=0;
                     ...
                     else if(b->key[y]==-1){cout<<"Q";b->key[y]=a1;b->child[y]=b1;b->child[y+1]=b2;break;}  
                       }
                       }
                  }    
             }
...
main()
{
      ...
      for(int i=0;i<a;i++)
      {
              ...
              p=0;cin>>c;b=insert(b,h1,c,d);h1=b;
              //cout<<"b1 and b2 are\n";print(b1,d);cout<<"A";print(b,d);cout<<"B";print(b2,d);cout<<endl;
      }//cout<<b->child[4]->key[3]<<endl;
      int nt=0;print(b,d);for(int j=0;j<=(2*d)-1;j++){if(b->key[j]!=-1){nt++;}}if(nt==2*d){print(b->child[2*d],d);}
      system("pause");
}
