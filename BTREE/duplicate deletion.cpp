...
int p=0,m=0,a1,k2;btree *b1=NULL,*b2=NULL,*x2,*z;
...
for(int t=s;t>0;t--)
{
    b->key[y+t]=b->key[y+t-1];cout<<y+t<<"T";if(g1==1){b->child[y+t+1]=b->child[y+t];}}
...
{int j;for(j=0;j<2*d;j++)
    {if(k<b->key[j]){break;}else if(b->key[j]==-1){break;}else if(k==b->key[j]){break;}}return j;
}
int k1;
...
{int j,l,p2,t2;
p2=cou(b->child[i],d);t2=b->child[i]->key[p2-1];b->child[i]->key[p2-1]=-1;
for(j=0;j<(2*d)-1;j++)
...
b->key[i-1]=t2;
for(j=0;j<(2*d)-1;j++){b->child[i]->key[j]=b->child[i]->key[j+1];}b->child[i]->key[(2*d)-1]=-1;
return b;
}
int y1;
...
p1=fn(b,k,d);cout<<p1<<"WENT ";
  ...
  {cout<<"OUT";if(b->child[i]!=NULL)
    ...
    if((cou(b->child[i],d)==2*d)||(tag==1))
    {
	  ...
	  {if((cou(b->child[i+1],d)>d)&&(tag==1)){cout<<"u";b=rit(b,d,i+1);k1=1;}//i+1 since it takes frm right
	  else if((cou(b->child[i+1],d)!=2*d)&&(tag==0))
	  {b=right(b,d,k,i);k1=1;}}break;}
      else if((b->key[i]==-1)||(i==2*d))
      {cout<<"DOG ";if((cou(b->child[i-1],d)>d)&&(tag==1))
	  {cout<<"n";b=lef(b,d,i-1);k1=1;}//i-1 since takes frm left
	  else if((cou(b->child[i-1],d)!=2*d)&&(tag==0)){b=left(b,d,k,i);k1=1;}break;}
      else {cout<<"PIG ";if((cou(b->child[i-1],d)&&(tag==1))>d){b=lef(b,d,i-1);k1=1;}
	  else if((cou(b->child[i-1],d)!=2*d)&&(tag==0)){b=left(b,d,k,i);k1=1;}
	  if((tag==1)&&(cou(b->child[i+1],d)>0)){b=rit(b,d,i+1);k1=1;}
          else if((b->child[i+1]!=NULL)&&(tag==0)){if(cou(b->child[i+1],d)!=2*d){b=right(b,d,k,i);k1=1;}}break;}
    }}
    }
  }return b;
}
...
btree *ln,*x;int x1,m1,z1,r;//ln=Leaf Node;z1 for repeating only once;
btree* merge(btree *b,btree *h1,int d,int i)
{int f,j,p1,p2,l,t1,p3;
p1=cou(b->child[i],d);cout<<p1;
b->child[i]->key[p1]=b->key[i];b->child[i]->child[0]->key[0];
p2=cou(b->child[i+1],d);p1++;cout<<p2<<p1;p3=p1;
for(j=i;j<(2*d)-1;j++){cout<<"F1";b->key[j]=b->key[j+1];}b->key[(2*d)-1]=-1;
for(j=0;j<p2;j++){cout<<"F1";b->child[i]->key[p1]=b->child[i+1]->key[j];p1++;}
for(l=i+1;l<2*d;l++){cout<<"F3";b->child[l]=b->child[l+1];}b->child[2*d]=NULL;
for(f=0;f<=p2;f++){cout<<"F4"<<f+p3<<p2;cout<<b->key[0]<<b->child[i]->key[0];
if(kou(b->child[i],d)!=0)
{cout<<"m";
         b->child[i]->child[f+p3]=b->child[i+1]->child[f];
}}
if((cou(b,d)!=0)){return b;}
else{cout<<"f";return (b->child[i]);}
}
...
if(b!=NULL){
           ...
           else
           {
               if(x1==0)
               {
                       x1++;b=b->child[i];p=cou(b,d);x=max(b,d,p-1);
               }
               else
               {
                       p=cou(b,d);p=cou(b,d);x=max(b->child[p-1],d,p-1);
               }
			   return x;
           }
       }
...
if(b!=NULL)
{
       for(i=0;i<2*d;i++)
       {
		   ...
		   if(k2==1)
		   {cout<<"found2";
              index=cou(b,d);s->key[s2]=b->key[index-1];cout<<s->key[s2]<<"&";
              b->key[index-1]=-1;m1=1;break;
           }
             if(k2==0)
              {cout<<"found1\n";
			  ln=max(b,d,i);s=b;s2=i;m1=1;
			  p3=cou(ln,d);cout<<p3<<ln->key[p3-1]<<"#";
			  if(kou(b,d)!=0){k2++;b->child[i]=search(b->child[i],h1,d,ln->key[p3-1]);break;}
			  else{index=cou(b,d);s->key[s2]=b->key[index-1];cout<<s->key[s2]<<"&";
              b->key[index-1]=-1;m1=1;break;}
			  }
           }
           else if((b->key[i]==-1)||(k<b->key[i]))
           {cout<<"DON";
                b->child[i]=search(b->child[i],h1,d,k);break;
           }
           else if(i==(2*d)-1)
           {cout<<"PIP";
                b->child[i+1]=search(b->child[i+1],h1,d,k);break;
           }
       }
	   if(m1==2)
	   {int q;
            if((kou(b,d)!=0)&&(z1!=1))
            {
              ...
              {cout<<"AOUT";
                  ...
                  if(i==0)
				  {cout<<"AGOD ";
                     if(b->child[i+1]!=NULL){if(cou(b->child[i+1],d)<=d){b=merge(b,h1,d,i);}}
					 break;
				  }
                  else if((b->key[i]==-1)||(i==2*d))
                  {cout<<"ADOG ";if(cou(b->child[i-1],d)<=d){b=merge(b,h1,d,i-1);}break;}
                  else
				  {cout<<"APIG ";if(cou(b->child[i-1],d)<=d){b=merge(b,h1,d,i-1);}
                       else if(b->child[i+1]!=NULL){if(cou(b->child[i+1],d)<=d){b=merge(b,h1,d,i);}}
			      break;
				  }
                  }
             }
...
{int count,kount,s=0;count=cou(b,d);kount=kou(b,d);
     if(b!=NULL)
     {
         ...
         {cout<<"p";
             if((kount!=0))
             {cout<<"t";k1=0;b=redib(b,d,k,0);cout<<"v"<<k1;if(k1==1){cout<<"$";break;}
             ...
             {int f1=fn(b,a1,d);
                  ...
                  {cout<<"c"<<a1;
                  k1=0;y1=1;if(m!=1){b=redib(b,d,a1,0);if(k==1){m=1;}}cout<<"v"<<k1;
if(k1!=1){
if(cou(b,d)==2*d){if(b==h1){split(b->child[f1],a1,d,0,2,3);cout<<"G";b=split(b,a1,d,1,2,3);m=1;}}
else{
p=3;cout<<"z";split(b->child[f1],a1,d,0,2,3);m=1;
if((cou(b->child[f1],d)==2*d)&&(cou(b,d)<=(2*d)-1)&&(k1!=1))
{if(f1==0){if(cou(b->child[f1+1],d)==2*d){cout<<"*";m=0;p=2;}}
else if((f1==2*d)||(b->key[f1]==-1)){(cou(b->child[f1-1],d)==2*d);{cout<<"*";m=0;p=2;}}
             ...
             {cout<<"eee";
                  ...
                  else if(b==h1){cout<<"G";b=split(b,a1,d,1,2,3);m=1;}
...
main()
{
      ...
      btree *h1,*R;h1=new btree;
      R=new btree;
      R->key=new int[2*d];
      for(int i=1;i<=(2*d)-1;i++){R->key[i]=-2;}R->child=NULL;
...
p=0;cout<<"b1 and b2 are\n";print(b1,d);cout<<"A";print(b,d);cout<<"B";
print(b2,d);cout<<endl;}print(b,d);
cout<<endl;//print2(b,d,R);
for(int i=0;i<a;i++)
{
              cout<<"enter the element to be deleted\n";
cin>>c;x1=0;m1=0;z1=0;k1=0;k2=0;b=search(b,h1,d,c);h1=b;print(b,d);
}
int tt;cin>>tt;print(b,d);
}
