...
hpnode* insert(hpnode *d,int b,int y)
{
     ...
     {
                d=new hpnode;
                d->data=b;d->parent=NULL;d->lchild=NULL;d->rchild=NULL;q.enqueue(d); q.enqueue(d);         
     }
     else
     {g=q.dequeue();
     if(g!=NULL)
     ...
     if((g->lchild==NULL)&&(y%2!=0))
         {e=new hpnode;
         e->parent=g;g->lchild=e;e->data=b;e->lchild=NULL;e->rchild=NULL;
         q.enqueue(g->lchild);q.enqueue(g->lchild);}
     if((g->rchild==NULL)&&(y%2==0))
         {e=new hpnode;
         e->parent=g;g->rchild=e;e->data=b;e->lchild=NULL;e->rchild=NULL;
		 q.enqueue(g->rchild);q.enqueue(g->rchild);}
     }}
     return d;
}
...
void print2(hpnode *b,hpnode *r)
{
     ...
     else{cout<<a1->data<<"  ";if(k>2){cout<<"par="<<a1->parent->data<<"  ";}}
     print2(a1,r);
     }
}
main()
