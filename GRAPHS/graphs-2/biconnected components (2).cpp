...
void dft1(int c,int d,adj A,node n[])
{
     ...
     for(i=0;i<d;i++)
	 {
      if(deg(n,A,d,c,1)==1){g1=0;}//if degree is 0 then the rest of o/p should be in another grp;
      if((A.b[c][i]!=0)&&(n[i].ap!=1)&&(n[i].vis!=1))
         {
             if((g1==0)&&(c==start)){cout<<"\n"<<n[c].data<<"K";g1++;}
			 cout<<n[i].data;A.b[c][i]=0;A.b[i][c]=0;
             n[i].vis=1;
             dft1(i,d,A,n);
         }
         else if((A.b[c][i]!=0)&&((n[i].ap!=1)||(n[c].ap!=1))&&(n[i].vis!=1))
         {A.b[c][i]=0;A.b[i][c]=0;
		  	  if(i!=start){n[i].vis=1;cout<<n[i].data<<"J\n";if(deg(n,A,d,start,1)!=1){g1=0;}}
		 else if((i==start)&&(deg(n,A,d,c,1)==1)){cout<<"H\n";if(deg(n,A,d,start,1)!=1){g1=0;}}
		 }
     }
}
...
{	ifstream fin;
	...
	char s;
	adj A;
	cout<<"enter the no. of vertices frm xx.cpp file\n";
	cin>>d;
	node n[d];
	input(n,d);
	for(i=0;i<d;i++)
