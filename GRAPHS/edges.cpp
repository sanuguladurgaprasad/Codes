...
//l1=tree;l2=fwd;l3=back;l4=trans;
void dft(int c,int d,adj A,node n[])
{
     ...
     {cout<<c<<"U";
		  ...
		  {cout<<"u"<<n[c].pn<<"\n"<<c<<r<<i<<"\n"<<n[i].pn<<"w\n";
		   	   if((n[i].pn>=r)&&(n[c].pn>=r)){cout<<"T";
		  if(n[c].pn<n[i].pn)
		  {l2=add(l2,n[c].data,n[i].data);e++;}
		  else if(n[c].pn>n[i].pn)
		  {l3=add(l3,n[c].data,n[i].data);}}
		   	   else {cout<<"K";l4=add(l4,n[c].data,n[i].data);}
     }
	 }
	 }
	 /*for(i=0;i<d;i++)
	  {
	        if(n[i].vis!=1)
	        {n[i].pn=++t;r1=n[i].data;
			r=n[find(n,n[i].data,d)].pn-1;dft(i,d,A,n);
	        }
	  }*/
}
