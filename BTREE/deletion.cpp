...
btree *ln,*x;
int x1,m1,r;//ln=Leaf Node;z1 for repeating only once;
						...
						{	cout<<"c"<<a1;
							...
							if(k1!=1) {
									if(cou(b,d)==2*d) {
											...
											p=3;
											cout<<"z";
											b->child[f1]=split(b->child[f1],a1,d,0,2,3);
											m=1;//cout<<cou(b->child[f1],d)<<cou(b,d)<<"T";
											if((cou(b->child[f1],d)==2*d)&&(cou(b,d)<=(2*d)-1)&&(k1!=1))
												{cout<<f1; if(f1==0)
													...
													else if((f1==2*d)||(b->key[f1]==-1)) {cout<<"F\n";
