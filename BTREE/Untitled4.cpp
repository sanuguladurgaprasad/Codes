...
int p=0,m=0,a1;btree *b1=NULL,*b2=NULL,*z,*x2;
...
if(b!=NULL)
{
  ...
  if(cou(b->child[i],d)<d)
    {
      if(i==0){cout<<"GOD ";
         if(b->child[i+1]!=NULL){if(cou(b->child[i+1],d)<=d){b=mergeright(b,d,i);}}break;}
      else if((b->key[i]==-1)||(i==2*d))
      {cout<<"DOG ";if(cou(b->child[i-1],d)<=d){b=mergeleft(b,d,i);}break;}
      else {cout<<"PIG ";if(cou(b->child[i-1],d)<=d){b=mergeleft(b,d,i);}
         else if(b->child[i+1]!=NULL){if(cou(b->child[i+1],d)<=d){b=mergeright(b,d,i);}}break;}
    }
  }
              }
            }
       ...
       if(m1==1)
       {
		  if((cou(b)>=d)&&(z!=1))
			{
			  for(int it=0;it<cou(b);it++)
			  {for(int in=0;in<cou(b)-1;in++)
                 {
                 if(b->key[in]>b->in[in+1])
     ...
     if(b!=NULL)
     {
         ...
         {cout<<"p";
             ...
             {int f1=fn(b,a1,d);
                  ...
                  {cout<<"c"<<a1;
                  k1=0;y1=1;if(m!=1){b=redib(b,d,a1);if(k==1){m=1;}}cout<<"v"<<k1;
if(k1!=1){
...
(b2,d);cout<<endl;}print(b,d);
/*for(int i=0;i<a;i++)
{g=0,j=0;
              cout<<"enter the element to be deleted\n";
cin>>c;x1=0;m1=0;z1=0;k1=0;b=search(b,h1,d,c,j,g);h1=b;}*/
