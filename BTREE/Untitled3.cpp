...
{	int j,l,t2,p2;
	...
	b->child[i-1]->child[p2]=b->child[i]->child[0];
	for(j=0; j<(2*d)-1; j++) {
			b->child[i]->child[j]=b->child[i]->child[j+1];
		}
		b->child[i]->child[j+1]=NULL;
	return b;
}
int y1;
						...
						{if((cou(b->child[i],d)==2*d)||(tag==1))
								{
									...
									else {	if((cou(b->child[i-1],d)>d)&&(tag==1)) {
													b=lef(b,d,i-1);
													k1=1;break;
												}
											else if((cou(b->child[i-1],d)!=2*d)&&(tag==0)) {
	...
	for(int i=0; i<=a; i++)
		{
			...
			b=search(b,h1,d,c);
			h1=b;print(b,d);
		}cout<<"all the elements got deleted\n";
	int tt;
	cin>>tt;
	print(b,d);
}
